/**
 * Music system for Conduit using Web Audio API.
...
class MusicSystem {
  ...
  private playNote(freq: number, startTime: number, duration: number, vol: number = 1): void {
    ...
    // Sine wave for smooth, flowing sound
    osc.type = 'sine';
  ...
  // Gameplay track - flowing, liquid, relaxing
  private getGameplayNotes(): Note[] {
    ...
    // Flowing water-like melody (8 bars)
    const melody = [
      // Bar 1-2: Gentle wave
      { note: 'E4', dur: eighth },
      { note: 'G4', dur: eighth },
